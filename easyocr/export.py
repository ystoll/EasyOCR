...
def export_detector(
    ...
    if detector:
        ...
        np.testing.assert_allclose(to_numpy(feature_torch_out), feature_onnx_out, rtol=1e-03, atol=1e-05)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="export detector onnx file path ending in .onnx" + "Do not pass in this flag to avoid exporting detector",
    )
    parser.add_argument("-d", "--dynamic", action="store_true", help="Dynamic  input output shapes for detector")
    parser.add_argument(
        "-is", "--in_shape", nargs="+", type=int, default=[1, 3, 608, 800], help="-is 1 3 608 800 (bsize, channel, height, width)"
    )
    parser.add_argument("-m", "--model_storage_directory", type=str, help="model storage directory for craft model")
    parser.add_argument("-u", "--user_network_directory", type=str, help="user model storage directory")
